...
export default function EngineerProfile() {
  ...
  // ============== FETCH INBOX FOR ENGINEER (PRIVATE CHATS) ==============
  const fetchInbox = async () => {
    try {
      ...
      console.log(`🔍 Fetching inbox for engineer: ${engineerId}`);

      // Engineer inbox requires recipientId query parameter
      const response = await axios.get(
        `${BASE_URL}/api/messages/inbox/engineer?recipientId=${engineerId}`,
      ...
      const formattedConversations = inboxData.map((item) => {
        return {
          ...
          customer_name: `Customer ${item.customer_id || item.id}`,
          last_message:
            item.last_message || item.latest_message || "No messages",
          last_message_time: item.last_message_time || item.updated_at,
  ...
  const loadSavedConversations = async () => {
    try {
      const saved = await AsyncStorage.getItem(
        `engineer_conversations_${engineerId}`,
      );
      if (saved) {
  ...
  const searchCustomer = async () => {
    ...
    try {
      ...
      // Engineer messages require recipientId query parameter
      const response = await axios.get(
        `${BASE_URL}/api/messages/${customerId}/engineer?recipientId=${engineerId}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const formatAndSetMessages = (messagesData) => {
    const formattedMessages = messagesData.map((msg, index) => {
      ...
      const isFromEngineer = msg.sender === "engineer";
      ...
      return {
        ...
        sender_name: isFromCustomer
          ? "Customer"
          : isFromEngineer
            ? userData?.name || "Engineer"
            : "Engineer",
        sender_role: msg.sender,
        created_at: msg.created_at,
        is_from_customer: isFromCustomer,
        is_from_engineer: isFromEngineer,
      };
    });
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !selectedConversation || !engineerId) return;
    ...
    const tempMessage = {
      ...
      sender_name: userData?.name || "Engineer",
      sender_role: "engineer",
      created_at: new Date().toISOString(),
      is_temp: true,
      is_from_customer: false,
      is_from_engineer: true,
    };
    ...
    try {
      ...
      // Engineer messages require recipient_id
      await axios.post(
        `${BASE_URL}/api/messages`,
        {
          ...
          recipient_id: engineerId, // Required for engineer messages
          message: messageText,
        },
        {
          headers: {
  ...
  const renderMessage = ({ item }) => {
    ...
    const isFromEngineer = item.is_from_engineer;
    const isMe = isFromEngineer;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatarContainer}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {getInitials(userData?.name || "Engineer")}
              </Text>
            </View>
          ...
          <Text style={styles.userName}>{userData?.name || "Engineer"}</Text>
          <Text style={styles.userEmail}>
            {userData?.email || "engineer@example.com"}
          </Text>

          <View style={styles.roleBadge}>
            <Ionicons name="car-outline" size={16} color="#10b981" />
            <Text style={styles.roleText}>Engineer</Text>
          </View>
          <View style={styles.engineerIdBadge}>
            <Ionicons name="card-outline" size={14} color="#64748b" />
            <Text style={styles.engineerIdText}>ID: {engineerId || "N/A"}</Text>
          </View>
        </View>
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoLabelContainer}>
                ...
                <Text style={styles.infoLabel}>Engineer ID</Text>
              </View>
              <Text style={styles.infoValue}>{engineerId || "Not set"}</Text>
            </View>
          </View>
        </View>
