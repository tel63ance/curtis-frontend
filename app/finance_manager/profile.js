// app/finance_manager/profile.js
...
export default function FinanceManagerProfile() {
  ...
  const fetchInbox = async () => {
    try {
      ...
      console.log("🔍 Fetching inbox for finance_manager...");

      // CORRECT ENDPOINT: /api/messages/inbox/finance_manager
      const response = await axios.get(
        `${BASE_URL}/api/messages/inbox/finance_manager`,
        {
  ...
  const loadSavedConversations = async () => {
    try {
      const saved = await AsyncStorage.getItem("finance_manager_conversations");
      if (saved) {
  ...
  const searchCustomer = async () => {
    ...
    try {
      ...
      // CORRECT ENDPOINT: /api/messages/:customerId/finance_manager
      const response = await axios.get(
        `${BASE_URL}/api/messages/${customerId}/finance_manager`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const formatAndSetMessages = (messagesData) => {
    const formattedMessages = messagesData.map((msg, index) => {
      ...
      const isFromService =
        msg.sender === "finance_manager" ||
        msg.sender_role === "finance_manager";
      ...
      >
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatarContainer}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {getInitials(userData?.name || "Finance Manager")}
              </Text>
            </View>
          ...
          <Text style={styles.userEmail}>
            {userData?.email || "finance@nairobidbotanica.com"}
          </Text>
