// app/inventory_manager/profile.js
...
export default function inventoryManagerProfile() {
  ...
  const fetchInbox = async () => {
    try {
      ...
      console.log("🔍 Fetching inbox for inventory_manager...");

      // CORRECT ENDPOINT: /api/messages/inbox/inventory_manager
      const response = await axios.get(
        `${BASE_URL}/api/messages/inbox/inventory_manager`,
        {
  ...
  const saveConversations = async (convs) => {
    try {
      await AsyncStorage.setItem(
        "inventory_manager_conversations",
  ...
  const searchCustomer = async () => {
    ...
    try {
      ...
      // CORRECT ENDPOINT: /api/messages/:customerId/inventory_manager
      const response = await axios.get(
        `${BASE_URL}/api/messages/${customerId}/inventory_manager`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const formatAndSetMessages = (messagesData) => {
    const formattedMessages = messagesData.map((msg, index) => {
      ...
      const isFromService =
        msg.sender === "inventory_manager" ||
        msg.sender_role === "inventory_manager";
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Inventory Manager Profile</Text>
      </View>
      ...
      >
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatarContainer}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {getInitials(userData?.name || "Inventory Manager")}
              </Text>
            </View>
          ...
          <Text style={styles.userEmail}>
            {userData?.email || "inventory@nairobidbotanica.com"}
          </Text>

          <View style={styles.roleBadge}>
            <Ionicons name="headset-outline" size={16} color="#2B5F3B" />
            <Text style={styles.roleText}>Inventory Manager</Text>
          </View>
        </View>
